...
test_that('model constructor runs without error'){
    ...
    foo@modelSteps[[2]]@phylo@e + 1
    foo@modelSteps[[3]]@phylo@e + 1
    foo@modelSteps[[2]]@localComm@indSpecies
    library(ape)
    foo@modelSteps[[2]]@phylo@e <- foo@modelSteps[[2]]@phylo@e + 1
    from <- foo@modelSteps[[2]]@phylo
    from
    plot(y)
    p <- ape::phylo(foo@modelSteps[[2]]@phylo)
    model@modelSteps[[1]]@localComm@indSpecies
    prepRebuild()
}
