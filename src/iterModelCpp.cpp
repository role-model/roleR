...
// update_env_filter_probs has not been made a function yet 
...
void call_dispersal(int i, int dead_index, int parent_indv, roleDataCpp &d, roleParamsCpp &p, bool print){
    ...
    int parent_index = parent_indv; //sample_index_using_probs(d.spAbundM);
...
void call_speciation(int i, int dead_index, roleDataCpp &d, roleParamsCpp &p, bool dispersed_this_iter, bool print){
    ...
    if(print){Rcout << "chosen species: " << chosen_species << "\n";}
...
void update_trait_diffs_sq(int dead_index, roleDataCpp &d, roleParamsCpp &p){
    ...
    for(int r = 0; r < p.individuals_local[0]; r++) //n_indv
    {
...
List iterModelCpp(RObject local, RObject meta, RObject phylo, RObject params, bool print) {
    ...
    //out[0] = out_d; //where the copy of role_data_from_cpp used to be
    ...
    for(int i = 0; i < (int) params.slot("niter"); i++) {
        ...
        if(R::runif(0,1) >= p.dispersal_prob(i)){
            ...
            // METHOD - call birth on data in place
        ...
        // NOTE - converting this to a test would be tricky as prev_env_sigma and 
        // env_filter_probs are defined earlier in the loop
        if(p.neut_delta[0] != 1)
        {
        ...
        if((i + 1) % niter_timestep == 0)
        {
            ...
            //NOTE - the copy of role_data_from_cpp used to be here
            ...
            if(print){Rcout << "created new data and added to out" << "\n";}
        }
    } 
...
// these avoid having to wrap each function individually 
// ONLY for testing 
