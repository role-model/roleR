...
for sp in meta_p.keys():
    ...
    # but we could pass a phylo that has all "t" prefixes
...
# note: this wonʻt work now because we changed migration rate calculation
...
# need to modify tree to have tip labels corresponding to spp numeric IDs
# but preceded by a character cause msprime canʻt make populations with number
# names
tre <- as(finalPhylo, "phylo")
tre$tip.label <- paste0("t", 1:Ntip(tre))
nwk <- write.tree(tre, digits = 18)
...
## Update on migration rate calculations

Instead of original idea of using the harmonic mean of the proportion, letʻs use the harmonic mean of the local community size.  Here are the steps for the new migration rate calculation:

$$
M_{local, meta}^{(i)} = \frac{m \alpha p^{(i)}_{meta} J_{harm} / 2}{\alpha h^{(i)}_{local}}
$$

Note: we added an $\alpha$ term to the numerator as well, because expected number of immigrants per timestep is actually $m \alpha p^{(i)}_{meta}$. We also replaced $J$ with $J_harm$ to represent the harmonic mean of $J$. Now some things cancel and simplify:

$$
M_{local, meta}^{(i)} = \frac{m p^{(i)}_{meta} J_{harm}}{2 h^{(i)}_{local}}
$$